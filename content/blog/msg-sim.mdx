...
ran into the necessity of emulting real networks with latency, jitter, packet
...
**msg-sim** is what we ended up building. It's a Rust library that creates
...
**tc/netem** is the foundation everything else builds on. The Linux kernel's
traffic control subsystem can add delay, loss, bandwidth limits, and more to any
network interface. The problem is that it operates at the system level. You run
`tc` commands to modify interfaces, and those changes affect everything using
that interface. There's no isolation between tests, no easy way to create
multi-peer topologies, and integrating it into a Rust test suite means shelling
...
connection, but it's not network emulation—it's application-layer proxying. Your
code has to connect through the proxy, which means changing connection strings.
It also can't simulate things like packet loss at the IP level or test UDP-based
protocols.
...
and lets you run async code inside them. We looked at it seriously. But it
appears to be unmaintained (limited commit activity), and it doesn't provide the
...
Impairments are dynamic—you can change them at runtime without recreating the
network:
...
With msg-sim, we can actually play around with these settings. Set up a 10
...
[In this example](https://github.com/chainbound/msg-rs/blob/main/msg-sim/examples/bdp_throughput.rs),
...
Under the hood, msg-sim creates a hub-and-spoke network topology using Linux
namespaces:
...
interface apply impairments based on destination IP—so Peer 1 can have different
latency to Peer 2 versus Peer 3.
...
This is all managed through direct netlink socket communication—no shelling out
to `tc` commands.
...
**Linux only.** The implementation uses namespaces, netlink, and tc. No macOS or
Windows support.

**Root required.** Creating namespaces and configuring tc needs `CAP_NET_ADMIN`.

**Single-threaded tests.** Parallel tests can conflict when manipulating
namespaces. We run with `--test-threads=1`.

## Closing Notes

`msg-sim` is still in alpha — the API and ergonomics are evolving as we use it
...
Check out the [API documentation](https://docs.rs/msg-sim) or open an issue on
