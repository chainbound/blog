...
BuilderNet's networking layer. This first collaboration aimed at bringing it to
...
uses JSON-RPC over HTTP. Assuming these two services keep running on the same
machine, we could implement a shared memory-based transport. While this would
clearly be a benefit compared to the status quo, we’re coming closer to a point
of diminishing returns compared to other improvements.

Therefore, next steps could move away from FlowProxy and concentrate on a
