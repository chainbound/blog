...
![Simplified diagram of BuilderNet topology](/introducing-flowproxy/topology.png)

<small>Simplified diagram of BuilderNet topology</small>
...
![Flamegraph image](/introducing-flowproxy/flamegraph.png)
...
![Flamegraph image](/introducing-flowproxy/flamegraph-ecdsa.png)
...
cache hit ratio of up to 70%**, meaning that _time spent validating transaction
signatures in bundles has been decreased by the same amount_.

![Cache hit ratio](/introducing-flowproxy/cache-hit-ratio.png)
...
![Graph](/introducing-flowproxy/proxy-v1-small-latency.png)

<small>Proxy v1, small requests (< 50KB). Range over 2 days: **137ms** **→ 3.6sec**</small>

![Graph](/introducing-flowproxy/proxy-v2-small-latency.png)

<small>Proxy v2, small requests (< 50KB). Range over 2 days: **85ms → 2sec**</small>

![Graph](/introducing-flowproxy/proxy-v1-big-latency.png)

<small>Proxy v1, big requests (> 50KB). Range over 2 days: **260ms → 2.8sec**</small>

![Graph](/introducing-flowproxy/proxy-v2-big-latency.png)

<small>Proxy v2, big requests (> 50KB). Range over 2 days: **85ms → 2sec**</small>
...
![Graph of connection attempts](/introducing-flowproxy/connections.png)
...
![Graph of connection attempts after](/introducing-flowproxy/connections-after.png)
In addition, the HTTP timeouts for long RTT links also decreased since then:

![image.png](/introducing-flowproxy/http-failures.png)

As well as p99 round-trip latencies on those links:

![Graph of p99 latency](/introducing-flowproxy/rpc-latency.png)

<small>Y-axis is logarithmic, p99 dropped from ~125ms to ~95ms.</small>
