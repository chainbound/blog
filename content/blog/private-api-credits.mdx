...
Privacy is back on the radar. Initiatives like
[Kohaku](https://ethereum.github.io/kohaku/) are springing up, the price of
Zcash increased 9-fold over the last year, and shielding protocols like
[Railgun](https://www.railgun.org/) have
[attracted a lot of liquidity](https://dune.com/railgun_project/railgun).
However, in a recent [report](https://pse.dev/blog/privacy-experience-report) by
PSE on the current privacy experience, many pain points that privacy-conscious
users experience were laid out. One concern stood out to us in particular: a
user setting up [Privacy Pools](https://privacypools.com/) locally was confused
that they had to configure an Alchemy API key to get the front-end running
locally.
...
full chain of requests made by a single account. The account is clearly needed
for purchasing API credits, and potentially any compliance checks. But is the
link between an API key and the PII really necessary? Additionally, do we really
need a persistent identifier across _all_ requests? Can it be avoided so users
can remain safe from honest-but-curious providers or data breaches? To answer
this question, we’ll need to take a detour and look at a fairly annoying but
ubiquitous aspect of browsing the internet: CAPTCHAs.

## CAPTCHAs & Privacy Pass

CAPTCHAs were introduced to identify requests coming from humans instead of
bots, used in website protection services like what Cloudflare offers. This was
a necessary service to protect websites from DoS attacks, but it introduced a
lot of friction for users. Millions of collective hours were wasted in solving
...
CAPTCHAs users had to solve online, while maintaining privacy. This last part is
important. Cloudflare could technically issue a persistent identifier to users
solving a CAPTCHA, that can be redeemed every time a user is presented another
challenge. But that would allow Cloudflare to trivially track the complete
browsing history of a user. Not great.
...
cryptographically unlinkable to the original blinded token, but the
