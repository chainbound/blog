...
beacon node serves as a _potential_ entrypoint into the network for validators,
but it is not representative of the actual validator distribution.

![](/validator-decentralization/meme.png)

<small>Probably not.</small>
...
A _validator client_ is the piece of software that executes these
...
_beacon node_, which is a standalone client that maintains the beacon chain and
communicates with other beacon nodes.

![](/validator-decentralization/node-schematic.png)

<small>Schematic of validator clients and a beacon node.</small>
...
blocks by broadcasting _attestations_. These attestations express the view of a
...
To address these issues, the beacon network is subdivided into _committees_,
...
logical components called _topics_ (aka P2P overlays)_._ These are the
...
topic, which is a _global topic_ on which new beacon blocks are broadcast. Every
...
subnets, how does that work? This is where _attestation aggregators_ come in.
These are a subset of the beacon committees that are responsible for
_aggregating_ all of the attestations they see and broadcasting the aggregate
...
![](/validator-decentralization/architecture.png)
...
https://github.com/chainbound/valtrack deployment that pushes all data to Dune
every 24 hours.

<aside>
💡 Dune table link: https://dune.com/chainbound/geolocating-validators

</aside>

_This data has been stripped of sensitive information such as IP addresses and
exact coordinates. However, it retains information like city, coordinates with a
precision of a 10km radius, and ASN information._
...
![](/validator-decentralization/map.png)
