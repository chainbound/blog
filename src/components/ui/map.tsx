...
type MapRouteProps = {
  ...
  onMouseEnter?: (lngLat: { lng: number; lat: number }) => void;
  /** Callback when mouse leaves the route line */
  onMouseLeave?: () => void;
  /** Whether the route is interactive - shows pointer cursor on hover (default: true) */
  interactive?: boolean;
  /** Width of invisible hit area for mouse events in pixels (default: 20) */
  hitAreaWidth?: number;
};
...
function MapRoute({
  ...
  // Use refs to avoid re-running event listener useEffect on callback changes
  ...
  useEffect(() => {
    ...
    // Add invisible hit area layer first (underneath)
    map.addLayer({
      id: hitLayerId,
      type: 'line',
      source: sourceId,
  ...
  // Handle click and hover events on the hit area layer
  useEffect(() => {
    ...
    const handleMouseEnter = (e: MapLibreGL.MapLayerMouseEvent) => {
      ...
      onMouseEnterRef.current?.({ lng: e.lngLat.lng, lat: e.lngLat.lat });
    };
    ...
    // Attach events to the wider hit area layer for easier interaction
