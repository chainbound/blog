...
function createStyledHeading(Component: typeof defaultMdxComponents.h1, sizeClass: string) {
    return function StyledHeading(props: ComponentProps<'h1'>) {
        return <Component {...props} className={`${props.className ?? ''} ${fontMono} ${sizeClass}`} />;
    };
}
...
export default async function Page(props: {
    ...
    return (
        <DocsPage toc={page.data.toc} tableOfContent={{ style: 'clerk', single: true }}>
            <BackLink />
            <DocsTitle className={`${fontMono} text-4xl`}>{page.data.title}</DocsTitle>
            <DocsDescription>{page.data.description}</DocsDescription>
            <div className={`flex flex-wrap items-center gap-4 text-sm text-fd-muted-foreground mb-4 ${fontMono}`}>
